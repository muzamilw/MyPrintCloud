...
(function () {
    ...
    */
    ...
    setBackgroundImage: function (image, callback, options) {
        if (image != null) { // updated by saqib to handle empty image
        ...
        if (typeof image === 'string') {
            fabric.util.loadImage(image, function (img) {
                ...
                if(property == "backgroundImage") {
                    ...
                    this[property].height = this.height;
                }
                callback && callback();
            }, this);
        }
    ...
    */
    renderAll: function (allOnTop) {
        ...
        if (activeGroup) {
            // adding open group panel by saqib 
    ...
    * @chainable
    */
    sendBackwardsWithS: function (object, sV) {
        var idx = this._objects.indexOf(object),
    ...
    /** added by saqib 
    * Moves an object  up in stack of drawn objects upto a particular index
    * @method bringForward
    * @param object {fabric.Object} Object to send
    * @return {fabric.Canvas} thisArg
    * @chainable
    */
    bringForwardWithS: function (object, sV) {
        var objects = this.getObjects(),
    ...
    /** added by saqib 
    * Returns object at specified index  upto a particular index
    * @method item
    * @param {Number} index
    * @return {fabric.Object}
    */
...
(function () {
    ...
    */
    _setObjectScale: function (localMouse, transform, lockScalingX, lockScalingY, by) {
        ...
        if (target.type != "i-text") { //added by saqib
            ...
            if (by === 'equally' && !lockScalingX && !lockScalingY) {
                ...
                if(transform.newScaleX> 0.02 ) 
                    lockScalingX || target.set('scaleX', transform.newScaleX);
                if(transform.newScaleY > 0.02 ) 
            ...
            target.maxWidth = target.width * target.scaleX;
            target.maxHeight = target.height * target.scaleY;
            this._flipObject(transform);
        } else {
            // added by saqib  for solving text issues
            var oldW = target.getWidth();
            var oldH = target.getHeight();

            if (by === 'x' && !target.lockUniScaling) {
...
(function (global) {
    ...
    */
    ...
    /**
    * ObjectId to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type int
    */
    ObjectID: 0,
        /**
    * undoID to be used in addition to state for undo manger// added by saqib
    * @property
    * @type int
    */
    undoID: 0,
        /**
* productPageID to be used in addition to state for undo manger// added by saqib
* @property
* @type int
*/
    productPageID: 0,
    /**
    * C of CMYK to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type int
    */
    C: 0,
    ...
    /**
    * M of CMYK to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type int
    */
    M: 0,
    ...
    /**
    * Y of CMYK to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type int
    */
    Y: 0,
    ...
    /**
    * K of CMYK to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type int
    */
    K: 0,
    ...
    /**
    * IsPositionLocked to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type boolean
    */
    ...
    /**
    * IsEditable to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type boolean
    */
    IsEditable: true,
    ...
    /**
    * IsHidden to be used in addition to state for tracking Id's// added by MZ
    * @property
    * @type boolean
    */
    IsHidden: false,
    ...
    */
    ...
    stateProperties: (
	  ...
	  'angle opacity fill fillRule shadow clipTo visible backgroundColor ObjectID'
	).split(' '),
...
(function () {
    ...
    */
    drawBorders: function (ctx) {
        ...
        /**
           * Draws borders of an object's bounding box to highlight the editable text. added by saqib
           * Requires public properties: width, height
           * Requires public options: padding, borderColor
           * @param {CanvasRenderingContext2D} ctx Context to draw on
           * @return {fabric.Object} thisArg
           * @chainable
           */
    drawBordersCustom: function (ctx,color) {
        if (!this.hasBorders) return this;
       ...
       ~ ~(h + padding2 + strokeWidth * this.scaleY) + 1
        ...
        ctx.textAlign = 'left';
        ctx.fillText("Edit", ~ ~(-(w / 2) - padding - strokeWidth / 2 * this.scaleX) - 0.5, ~ ~(-(h / 2) - padding - strokeWidth / 2 * this.scaleY) - 0.5 -2);
         //   
...
(function (global) {
    ...
    */
    ...
    /**
    * @private
    * @param {Object} [options] Options   // modified by saqib
    */
    ...
    */
    ...
    /**
    * @private
    * @param {CanvasRenderingContext2D} ctx Context to render on   updated by saqib
    */
...
(function (global) {
    ...
    */
    ...
    /**
    * Renders image on a specified context  // modified by saqib
    * @param {CanvasRenderingContext2D} ctx Context to render on
    * @param {Boolean} [noTransform] When true, context is not transformed
    */
    render: function (ctx, noTransform) {
        ...
        this._render(ctx, this.ImageClippedInfo);
        if (this.shadow && !this.shadow.affectStroke) {
            this._removeShadow(ctx);
        }
        this._renderStroke(ctx);
    ...
    */
    ...
    /**
    * @private
    * @param {CanvasRenderingContext2D} ctx Context to render on  // modified by saqib to enable image clipping imageclippedinfo
    */
    _render: function (ctx, xml) {
        if (xml != null && xml != undefined && xml != '') {
          ...
          //  e.drawImage(img, sx, sy, swidth, sheight, 0, 0, width, height);
        } else {
...
(function (global) {
    ...
    * @param {CanvasRenderingContext2D} ctx Context to render on
    */
    _render: function (ctx,objStyles) {
        var isInPathGroup = this.group && this.group.type === 'path-group';
        ...
        if (typeof Cufon === 'undefined' || this.useNative === true) {
            this._renderViaNative(ctx,objStyles);
        }
        else {
    ...
    /**
    * @private  // perform clipping
    * @param  //added by saqib  some functions copied from itext
    */
    _getCacheProp: function (_char, styleDeclaration) {
     ...
     isEmptyStyles: function () {
        if (!this.customStyles) return true;
        var obj = this.customStyles;
        ...
        _applyCharStylesGetWidth: function (ctx, _char, lineIndex, charIndex, decl,fontFamily,fontColor,fontSize,fontWeight,fontStyle) {
    ...
    */
    _applyFontStyles: function (styleDeclaration,fontDeclaration,fontSize,fontWeight,fontStyle) {
        if(fontDeclaration == undefined || fontDeclaration == null || fontDeclaration =="") {
            styleDeclaration.fontFamily = this.fontFamily;
        } else {
            styleDeclaration.fontFamily = fontDeclaration;
        }
        ...
        if(fontWeight == undefined || fontWeight == null || fontWeight =="") {
         ...
         if(fontStyle == undefined || fontStyle == null || fontStyle =="") {
            ...
            styleDeclaration.fontStyle = fontStyle;
        }
       
    },
      GetCharIndexFromLineIndex: function (lineIndex,charIndex) {
        var ctIndex = 0;
        var textlines = this.clippedText.split('\n');
        for(var i= 0; i <lineIndex;i++) {
            ctIndex += textlines[i].length +1;
        }
        ctIndex += charIndex;
        return ctIndex;
    ...
    */
    _getWidthOfChar: function (ctx, _char, lineIndex, charIndex) {
        if(this.customStyles == undefined )  {
            this.customStyles = [];
        }
        ctx.save();
        var cIndex = this.GetCharIndexFromLineIndex(lineIndex,charIndex);
        ...
        var width = this._applyCharStylesGetWidth(ctx, _char, lineIndex, charIndex ,[],objStyle['font-family'],objStyle['color'],objStyle['font-Size'],objStyle['font-Weight'],objStyle['font-Style']);
        ctx.restore();
        return width ;
    },
    ...
    */
    _getHeightOfChar: function (ctx, _char, lineIndex, charIndex) {
        var cIndex = this.GetCharIndexFromLineIndex(lineIndex,charIndex);
        var objStyles = this.customStyles[cIndex];
        if (objStyles) {
            return objStyles['font-Size']  || this.fontSize;
        }
    ...
    */
    _getWidthOfCharAt: function (ctx, lineIndex, charIndex, lines) {
        lines = lines || this.clippedText.split(this._reNewline); //this.text.split(this._reNewline);
    ...
    */
        ...
        for (var nz = 0; nz < sentence.length; nz++) {
            ...
            for (var n = 0; n < words.length; n++) {
                ...
                if(this.customStyles != null && this.customStyles != undefined  && this.customStyles.length != 0 && !this.isEmptyStyles())
                {
                    ...
                    spacingWidth = testLine.length * charSpacing;
                    renderFast = true;
                } 
               
                var testWidth = maxWidthLastLine + spacingWidth;
    ...
    _performClipping: function (ctx, text, objThis) {
        var formattedlines = [];
        formattedlines = this.wrapText(ctx, text, 0, 0, this.maxWidth, this.lineHeight * this.fontSize, this.maxHeight, this.charSpacing, "left", this.fontSize, this.customStyles, objThis);
    ...
    * @param {CanvasRenderingContext2D} ctx Context to render on
    */
    _renderViaNative: function (ctx,styles) {

        this.transform(ctx, fabric.isLikelyNode);

        this._setTextStyles(ctx);
        this.customStyles = styles;
        this.width = this.maxWidth; //this._getTextWidth(ctx, textLines);
        this.height = this.maxHeight; // this._getTextHeight(ctx, textLines);
        ...
        this.clipTo && ctx.restore();
        if (!this.selected && this.IsEditable != false && this.ObjectID > 0) { 
    ...
    _setBoundaries: function (ctx, textLines) {
        ...
        for (var i = 0, len = textLines.length; i < len; i++) {
            ...
            this._boundaries.push({
                height: this.maxHeight, //this.fontSize * this.lineHeight,
                width: this.maxWidth, //lineWidth,
                left: lineLeftOffset
            });
        }
    },
    ...
    _renderChars: function (method, ctx, chars, left, top) {
        ...
        if(this.textAlign == "center") {
            ...
            if(chars[chars.length-1] != " ")  {
                m = ctx.measureText(chars);
                lc = (chars.length-1) * charSpacing ;
            } 
            Cleft = left -(m.width/2)  - (lc/2); 
            ctx.textAlign = "left";
            ...
            for(var i = 0; i< chars.length;i++) {
                ...
                Cleft = Cleft + charWidth + (charSpacing);
            }
    },
    ...
    */
    _getFontDeclaration: function () {
        ...
        /**
    * @private added by saqib
    */
    _getFontDeclarationcustom: function (fontDeclaration,fontSize,fontStyle,fontWeight) {
        if(fontDeclaration == undefined || fontDeclaration == null || fontDeclaration =="") {
            fontDeclaration = this.fontFamily;
        }
         if(fontSize == undefined || fontSize == null || fontSize =="") {
		...
		fontSize + 'px',
		(fabric.isLikelyNode ? ('"' + fontDeclaration + '"') : fontDeclaration)
	  ].join(' ');
    },
    /**
    ...
    */
    toSVG: function (reviver) {
        var markup = [],
		  textLines = this.clippedText.split(this._reNewline), //this.text.split(this._reNewline),
...
(function () {
    ...
    */
    styles: null,
      ...
      /**
    * Object containing character styles   single array added by saqib
    * @type Object
    * @default
    */
    customStyles: null,
    ...
    */
    initialize: function (text, options) {
        ...
        this.customStyles = options ? (options.customStyles || {}) : {};
    ...
    */
    ...
    /**
    * Sets selection end (right boundary of a selection) added by saqib
    ...
    */
    getSelectionStyles: function (startIndex, endIndex) {
        ...
        return this.customStyles[this.GetCharIndexFromLineIndex(loc.lineIndex,loc.charIndex)] || {};
    ...
    */
    setSelectionStyles: function (styles) {
        if (this.selectionStart === this.selectionEnd) {
            this._extendStyles(this.selectionStart, styles,true,true);
        }
        else {
            var allItalic = false;
            var allBold = false;
            for (var i = this.selectionStart; i < this.selectionEnd; i++) {
                ...
                var cIndex = this.GetCharIndexFromLineIndex(loc.lineIndex,loc.charIndex);
                if (this.customStyles[cIndex]) {
                    if( this.customStyles[cIndex]['font-Style'] == 'italic') {
                        allItalic = true;
                    }
                    if(this.customStyles[cIndex]['font-Weight'] == 'bold') {
                        allBold = true;
                    }
                }
            }
            for (var i = this.selectionStart; i < this.selectionEnd; i++) {
                this._extendStyles(i, styles,allItalic,allBold);
            }
        }
        return this;
    },
    ...
    * @private
    */
    _extendStyles: function (index, styles,allItalic,allBold) {
...
//        }
        ...
        fabric.util.object.extend(this.customStyles[cIndex], styles);
    },
    ...
    */
    _render: function (ctx) {
        ...
        this.callSuper('_render', ctx , this.customStyles);
        this.ctx = ctx;
    ...
    */
    renderCursorOrSelection: function () {
        ...
        var chars = this.clippedText.split(''),
		  boundaries;
        if (this.selectionStart === this.selectionEnd) {
    ...
    */
    get2DCursorLocation: function (selectionStart) {
        ...
        var textBeforeCursor = this.clippedText.slice(0, selectionStart);
        var linesBeforeCursor = textBeforeCursor.split(this._reNewline);   // textBeforeCursor.split(this._reNewline);
        return {
    ...
    */
    getCurrentCharFontSize: function (lineIndex, charIndex) {
        var cIndex = this.GetCharIndexFromLineIndex(lineIndex,charIndex);
        if(this.customStyles[cIndex]) {
		    return (this.customStyles[cIndex]['font-Size']  || this.fontSize);
        }
        return this.fontSize
    },
    ...
    */
    getCurrentCharColor: function (lineIndex, charIndex) {
         var cIndex = this.GetCharIndexFromLineIndex(lineIndex,charIndex);
        if(this.customStyles[cIndex]) {
		    return (this.customStyles[cIndex]['color'] || this.fill);//cursorColor);
        }
        return this.fill;//this.cursorColor
    },
    ...
    */
    renderSelection: function (chars, boundaries) {
        ...
        for (var i = this.selectionStart; i < this.selectionEnd; i++) {
            ...
            else if (i !== this.clippedText.length) {
    ...
    * @param {Array} textLines Array of all text lines
    */
    _renderTextLinesBackground: function (ctx, textLines) {  // commented by saqib because we have to change styles to custom styles in it 
...
//        ctx.restore();
    ...
    */
    ...
    /**
    * @private
    * @param {CanvasRenderingContext2D} ctx Context to render on updated by saqib to adjust font size , color and font
    * @param {String} _char
    ...
    * @param {CanvasRenderingContext2D} ctx Context to render on
    */
    _getTextLines: function (ctx, text, maxheight, maxwidth) {
        var dummyLines = this.clippedText.split(this._reNewline);
        for (var i = 0; i < dummyLines.length; i++) {
            dummyLines[i] += '\n';
        }
        return dummyLines;
    },
    ...
    */
    toObject: function (propertiesToInclude) {
        return fabric.util.object.extend(this.callSuper('toObject', propertiesToInclude), {
            customStyles: clone(this.customStyles)
        });
    }
});
...
//        }, this.cursorDelay);
    ...
    */
    selectAll: function () {
        ...
        this.selectionEnd = this.clippedText.length;
    ...
    */
    getSelectedText: function () {
        return this.clippedText.slice(this.selectionStart, this.selectionEnd);
    },
        /**
           * Returns selected text
           * @return {String}
           */
    getSelectedTextActual: function () {
    ...
    */
    findWordBoundaryLeft: function (startFrom) {
        ...
        if (this._reSpace.test(this.clippedText.charAt(index))) {
            while (this._reSpace.test(this.clippedText.charAt(index))) {
                offset++;
                index--;
            }
        }
        while (/\S/.test(this.clippedText.charAt(index)) && index > -1) {
            offset++;
            index--;
        }
    ...
    */
    findWordBoundaryRight: function (startFrom) {
        ...
        while (/\S/.test(this.clippedText.charAt(index)) && index < this.clippedText.length) {
            offset++;
            index++;
        }
    ...
    */
    findLineBoundaryLeft: function (startFrom) {
        ...
        while (!/\n/.test(this.clippedText.charAt(index)) && index > -1) {
            offset++;
            index--;
        }
    ...
    */
    findLineBoundaryRight: function (startFrom) {
        ...
        while (!/\n/.test(this.clippedText.charAt(index)) && index < this.clippedText.length) {
            offset++;
            index++;
        }
    ...
    */
    searchWordBoundary: function (selectionStart, direction) {
        ...
        var _char = this.clippedText.charAt(index);
        var reNonWord = /[ \n\.,;!\?\-]/;

        while (!reNonWord.test(_char) && index > 0 && index < this.clippedText.length) {
            index += direction;
            _char = this.clippedText.charAt(index);
        }
    ...
    */
    enterEditing: function () {
        ...
        if (IsCalledFrom == 2 || IsCalledFrom == 1) {
            pcL36('show', '#divVariableContainer');
    ...
    */
    _updateTextarea: function () {
        ...
        this.hiddenTextarea.value = this.clippedText;
    ...
    * @private
    */
    _removeExtraneousStyles: function () { //updated by saqib
    if (IsInputSelected) return;
       // var textLines = this.clippedText.split(this._reNewline); //this.text.split(this._reNewline);
        var count = this.clippedText.length;
        //for (var i = 0; i < this.clippedText.length; i++) {
        //    count += textLines[i].length;
        //} 
        for (var prop in this.customStyles) {
            if (prop > count) {
                delete this.customStyles[prop];
            }
        }
    },
    ...
    */
    _removeCharsFromTo: function (start, end) {
        ...
        this.clippedText = this.clippedText.slice(0, start) +
				  this.clippedText.slice(end);
        this._calcTextFromClipped(start,end);
        
    },
     /**
    * @private   added by saqib
    */
    _calcTextFromClipped: function (start,end) {
    ...
    * @private   added by saqib
    */
    _calcTextFromClippedRemoveChar: function (oldT) {
    ...
    * @private   added by saqib
    */
    _calcTextFromClippedInsertChars: function (_chars) {        
            this.text = this.text.slice(0, this.selectionStart) +
					_chars +
    ...
    */
    insertChars: function (_chars) {
        ...
        this.clippedText = this.clippedText.slice(0, this.selectionStart) +
					_chars +
				  this.clippedText.slice(this.selectionEnd);
       
        if (this.selectionStart === this.selectionEnd) {
            this.insertStyleObjects(_chars, isEndOfLine, this.copiedStyles,1);
        }
        else if (this.selectionEnd - this.selectionStart > 1) {
            // TODO: replace styles properly  
           
            this.insertStyleObjectsExtension(_chars, isEndOfLine, this.copiedStyles, this.selectionEnd - this.selectionStart);
    ...
    */
    insertNewlineStyleObject: function (lineIndex, charIndex, isEndOfLine,clenght) {
     var cIndex = this.GetCharIndexFromLineIndex(lineIndex,charIndex) ;
...
//            this.styles[lineIndex + 1] = {};
     ...
     if (clenght != null) {
         if(parseInt(clenght)) {
             displacement = parseInt(clenght);
         }
     }
        var currentCharStyle = this.customStyles[cIndex-1],
		  newLineStyles = {};

        var allStyles = this.customStyles,
            allStylesCloned = clone(allStyles),
            allStylesToRestore = [];
        var addedIndex = this.GetCharIndexFromLineIndex(lineIndex, charIndex) - displacement;
       
        for (var line in this.customStyles) {
            if (line > addedIndex) {
                allStylesToRestore[parseInt(line) + displacement] = allStylesCloned[line];
            } else {
                allStylesToRestore[line] = allStylesCloned[line];
            }
        }
        this.customStyles = allStylesToRestore;
        // if there's nothing after cursor,
        // we clone current char style onto the next (otherwise empty) line
        //if (isEndOfLine) {
        //    //newLineStyles[0] = clone(currentCharStyle);
        //    //this.customStyles[cIndex + 1] = newLineStyles;
        //}
        ...
        //            delete this.customStyles[cIndex];
        //        }
        //    }
        //    this.customStyles[cIndex + 1] = newLineStyles;
        //}
    ...
    */ 
    insertCharStyleObject: function (lineIndex, charIndex, style, cLength,chars) {
        var displacement = 1;
        if (cLength != null) {
            if (parseInt(cLength)) {
                displacement = parseInt(cLength);
            }
        }
       
        var cIndex = this.GetCharIndexFromLineIndex(lineIndex, charIndex);
        if (chars.length != 1) {
            displacement -= chars.length; 
            if (cLength != 1) {
                displacement += 1;
            }
        }
        ...
        var allStyles = this.customStyles,
        allStylesCloned = clone(allStyles);
        var allStylesToRestore = [];
        if (cIndex === 0 && !style) {
            cIndex = 1;
        }
        if (displacement != 1) {
            cIndex = this.selectionStart;
            for (var line in allStylesCloned) {
                if (line > cIndex) {
            ...
            for (var index in allStylesCloned) {
                var numericIndex = parseInt(index, 10);
                if (index >= cIndex) {
                    allStylesToRestore[numericIndex + 1] = allStylesCloned[numericIndex];
                    //delete currentLineStyles[index];
                } else {
                    allStylesToRestore[numericIndex] = allStylesCloned[numericIndex];
                }
            }
            this.customStyles = allStylesToRestore;
            this.customStyles[cIndex] =
              style || clone(allStyles[cIndex - 1]);
        }
    ...
    /**
    * Inserts style object(s)  // modified by saqib
    * @param {String} _chars Characters at the location where style is inserted
    * @param {Boolean} isEndOfLine True if it's end of line
    * @param {Array} [styles] Styles to insert
    */
    insertStyleObjects: function (_chars, isEndOfLine, styles,cLenght) {
        // short-circuit
        ...
        if (_chars === '\n') {
            ...
            this.insertNewlineStyleObject(lineIndex, charIndex, isEndOfLine, cLenght);
        }
        else {
            if (styles) {
                this._insertStyles(styles, cLenght);
            }
            else {
                // TODO: support multiple style insertion if _chars.length > 1
                this.insertCharStyleObject(lineIndex, charIndex, null, cLenght,_chars); 
            }
        }
    },
        /**
    * Inserts style object(s) to replace mutliple selection   added by saqib 
    * @param {String} _chars Characters at the location where style is inserted
    * @param {Boolean} isEndOfLine True if it's end of line
    * @param {Array} [styles] Styles to insert
    */
    insertStyleObjectsExtension: function (_chars, isEndOfLine, styles,cindex) {
        // short-circuit
        //if (this.isEmptyStyles()) return;

        ////this.removeStyleObjectsss(cindex);
        //for (var i = 0; i < _chars.length; i++) {
        //    this.insertStyleObjects(_chars[i], isEndOfLine, this.copiedStyles);
        //}
        this.insertStyleObjects(_chars, isEndOfLine, styles, cindex);
        
    },
    /**
    * @private
    */
    _insertStyles: function (styles,cLength) {
        for (var i = 0, len = styles.length; i < len; i++) {
            ...
            this.insertCharStyleObject(lineIndex, charIndex, styles[i], cLength);
        }
    },
        ...
        var cursorLocation = this.get2DCursorLocation(index),
		  ...
		  charIndex = cursorLocation.charIndex;
           var textLines = this.clippedText.split(this._reNewline), // this.text.split(this._reNewline),
            ...
            allStylesCloned = clone(allStyles),
            allStylesToRestore =[];
            var charIndexRemoved = 0;
            for(var i=0; i < lineIndex;i++) {
                charIndexRemoved+= textLines[i].length;
            }
            var removedIndex = this.GetCharIndexFromLineIndex(cursorLocation.lineIndex, cursorLocation.charIndex) -1;
            if (this.customStyles[removedIndex]) {
                delete this.customStyles[removedIndex];
            }
            for (var line in this.customStyles) {
                if (line > removedIndex) {
                    allStylesToRestore[line - 1] = allStylesCloned[line];
                } else {
                    allStylesToRestore[line] = allStylesCloned[line];
                }
            }
            ...
            //for(var line in this.customStyles) {
            //     var numericLine = parseInt(line, 10);
            //    if (numericLine > charIndexRemoved) {
            //        allStylesToRestore[numericLine -1] = allStylesCloned[numericLine];
            //    } else {
            //        allStylesToRestore[numericLine] = allStylesCloned[numericLine];
            //    }
            //}
            //this.customStyles = allStylesToRestore;
    },
        // added by saqib to remove multiple styled objects
    removeStyleObjectsss: function (charLength) {
        var cursorLocation = this.get2DCursorLocation();
        var textLines = this.clippedText.split(this._reNewline);
        var allStyles = this.customStyles,
        allStylesCloned = clone(allStyles),
        allStylesToRestore = [];
        ...
        for (var line in this.customStyles) {
            if (line > removedIndex) {
                allStylesToRestore[line - charLength] = allStylesCloned[line];
            } else {
                allStylesToRestore[line] = allStylesCloned[line];
            }
        }
...
getSelectionStartFromPointer: function (e) {
    ...
    if (typeof newSelectionStart === 'undefined') {
        return this.clippedText.length;
    }
},
...
_getNewSelectionStartFromOffset: function (mouseOffset, prevWidth, width, index, jlen) {
    ...
    if (newSelectionStart > this.clippedText.length) {
        newSelectionStart = this.clippedText.length;
    }
...
initHiddenTextarea: function () {
    ...
    //this.hiddenTextarea.setAttribute('class', 'hiddenTextAreaCanvas');
    var cssText = 'position: absolute; top: 0; left: -9999px; width:' + this.maxWidth + 'px; Height:' + this.maxHeight + 'px;';
    this.hiddenTextarea.style.cssText = cssText; //'position: absolute; top: 0; left: -9999px';
...
*/
copy: function () {
    var selectedText = this.getSelectedTextActual();
...
getDownCursorOffset: function (e, isRight) {
    ...
    var selectionProp = isRight ? this.selectionEnd : this.selectionStart,
		...
		textBeforeCursor = this.clippedText.slice(0, selectionProp),
		textAfterCursor = this.clippedText.slice(selectionProp),
    ...
    if (cursorLocation.lineIndex === textLines.length - 1 || e.metaKey) {
        ...
        return this.clippedText.length - selectionProp;
    }
...
moveCursorDownWithoutShift: function (offset) {
    ...
    if (this.selectionStart > this.clippedText.length) {
        this.selectionStart = this.clippedText.length;
    }
    this.selectionEnd = this.selectionStart;
},
...
moveCursorDownWithShift: function (offset) {
    ...
    if (this._selectionDirection === 'left' && (this.selectionStart !== this.selectionEnd)) {
        ...
        if (this.selectionEnd > this.clippedText.length) {
            this.selectionEnd = this.clippedText.length;
        }
    }
},
...
getUpCursorOffset: function (e, isRight) {
    ...
    var textBeforeCursor = this.clippedText.slice(0, selectionProp),
		textOnSameLineBeforeCursor = textBeforeCursor.slice(textBeforeCursor.lastIndexOf('\n') + 1),
...
moveCursorLeftWithShift: function (e) {
    if (this._selectionDirection === 'right' && this.selectionStart !== this.selectionEnd) {
        ...
        if (this.clippedText.charAt(this.selectionStart) === '\n') {
            this.selectionStart--;
        }
        if (this.selectionStart < 0) {
            this.selectionStart = 0;
...
moveCursorRight: function (e) {
    if (this.selectionStart >= this.clippedText.length && this.selectionEnd >= this.clippedText.length) return;
...
moveCursorRightWithShift: function (e) {
    if (this._selectionDirection === 'left' && this.selectionStart !== this.selectionEnd) {
        ...
        // increase selection by one if it's a newline
        if (this.clippedText.charAt(this.selectionEnd - 1) === '\n') {
            this.selectionEnd++;
        }
        if (this.selectionEnd > this.clippedText.length) {
            this.selectionEnd = this.clippedText.length;
...
_removeCharsNearCursor: function (e) {
    ...
    if (this.selectionStart !== 0) {
        ...
        if (e.metaKey) {
            ...
            var isBeginningOfLine = ((this.clippedText.slice(this.selectionStart - 1, this.selectionStart) === '\n')); // || ( this.clippedText.slice(this.selectionStart - 1, this.selectionStart) === '\r\n') );
            this.removeStyleObject(isBeginningOfLine);

            this.selectionStart--;
            this.clippedText = this.clippedText.slice(0, this.selectionStart) +
					this.clippedText.slice(this.selectionStart + 1);
            this._calcTextFromClippedRemoveChar(this.clippedText);
        }
    }
}
});
