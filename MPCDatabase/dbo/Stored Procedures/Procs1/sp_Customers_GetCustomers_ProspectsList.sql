CREATE PROCEDURE dbo.sp_Customers_GetCustomers_ProspectsList
...
AS
 if(@CompanyLevel=0) 
	BEGIN 
         ...
         WHERE tbl_customers.SystemSiteID=@SystemSiteID AND tbl_customers.IsCustomer=1 AND tbl_customers.IsDisabled=0 Order By CustomerName ASC 
...
ELSE
	...
	SELECT  tbl_customers.CustomerID,tbl_customers.CustomerName,tbl_customers.AccountNumber,
         ...
         INNER JOIN tbl_customercontacts ON (tbl_customers.CustomerID = tbl_customercontacts.CustomerID AND tbl_customercontacts.IsDefaultContact=1)
         INNER JOIN tbl_company_sites ON (tbl_company_sites.SiteID=tbl_customers.SystemSiteID) 
         WHERE tbl_company_sites.CompanyID=@SystemCompanyID AND tbl_customers.IsCustomer=1 AND tbl_customers.IsDisabled=0 Order By CustomerName ASC 
