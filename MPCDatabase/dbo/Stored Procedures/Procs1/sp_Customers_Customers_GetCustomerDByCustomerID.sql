CREATE PROCEDURE [dbo].[sp_Customers_Customers_GetCustomerDByCustomerID]
...
AS
	SELECT tbl_ContactCompanies.ContactCompanyID,tbl_ContactCompanies.AccountNumber,
      ...
      -- tbl_ContactCompanies.IsAllowWebAccess,
       ...
       ,tbl_ContactCompanies.SalesPerson,IsNULL(tbl_SystemUsers.FullName,'') as FullName
	   ...
	   Left outer Join tbl_SystemUsers ON tbl_contactCompanies.NotesLastUpdatedBy = tbl_SystemUsers.SystemUserID
       WHERE (tbl_ContactCompanies.ContactCompanyID = @CustomerID and tbl_Addresses.IsDefaultAddress<>0)
	RETURN