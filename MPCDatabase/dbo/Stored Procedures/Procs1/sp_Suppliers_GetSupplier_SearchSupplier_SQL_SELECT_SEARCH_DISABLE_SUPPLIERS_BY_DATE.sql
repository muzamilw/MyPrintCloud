CREATE PROCEDURE dbo.sp_Suppliers_GetSupplier_SearchSupplier_SQL_SELECT_SEARCH_DISABLE_SUPPLIERS_BY_DATE
	(
		@SearchText varchar,
		@sDate datetime,
	    @eDate datetime
...
AS
	SELECT distinct tbl_suppliers.SupplierID,tbl_suppliers.SupplierName,
      ...
      AND tbl_suppliers.IsDisabled=1 OR tbl_suppliers.URL LIKE @SearchText OR tbl_suppliers.Terms LIKE @SearchText
      ...
      AND (tbl_suppliers.SupplierCreationDate BETWEEN @sDate AND @eDate) 
      OR (tbl_suppliers.AccountOpenDate BETWEEN @sDate AND @eDate)
	RETURN