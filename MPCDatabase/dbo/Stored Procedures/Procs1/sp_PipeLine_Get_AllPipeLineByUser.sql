...
Create PROCEDURE [dbo].[sp_PipeLine_Get_AllPipeLineByUser]
	@SalesPerson int
AS
		SELECT cast(tbl_items.ItemID as char) as ItemID,
	/*cast(tbl_estimates.Estimate_Code as char) as Estimate_Code,*/
	...
	tbl_estimates.SuccessChanceID,tbl_success_chance.Percentage,tbl_success_chance.Description as SuccessChanceDescription,
	(CASE WHEN tbl_estimates.IsEstimate<>0 THEN 0 ELSE 2 END)as IsProjection,tbl_items.IsIncludedInPipeLine as IsIncludedInPipeLine,tbl_estimates.SourceID,tbl_estimates.ProductID 
	FROM tbl_estimates 
	...
	and tbl_estimates.SalesPersonID = @SalesPerson
         UNION 
        select cast(tbl_estimate_projection.ProjectionID as char) as ItemID,cast('0' as Char) as Estimate_Code,cast('0' as char)as Estimate_Name,
tbl_estimate_projection.EstimateDate as ProjectionDate,0 as EstimateID,'0' as ItemCode,
'0' as ItemTitle,tbl_contactcompanies.Name,tbl_estimate_projection.Amount,
tbl_estimate_projection.SuccessChanceID,tbl_success_chance.Percentage,tbl_success_chance.Description as SuccessChanceDescription,
1 as IsProjection,tbl_estimate_projection.IsIncludedInPipeLine as IsIncludedInPipeLine,tbl_estimate_projection.SourceID,tbl_estimate_projection.ProductID 
from tbl_estimate_projection 
        ...
        where tbl_estimate_projection.SalesPerson = @SalesPerson ORDER BY Estimate_Code
	RETURN