CREATE PROCEDURE [dbo].[sp_Suppliers_GetSupplier_All]
	(
		@SystemSiteID int,
		@SystemCompanyID int,
...
where tbl_ContactCompanies.SystemSiteID=@SystemSiteID AND tbl_ContactCompanies.IsDisabled=0 AND tbl_ContactCompanies.Iscustomer = 2  ;
			END
       ELSE
		 ...
		 (tbl_Contacts.Title + ' '+ tbl_Contacts.FirstName  + ' '+  tbl_Contacts.MiddleName  + ' '+  tbl_Contacts.LastName) as FullName,
				...
				INNER JOIN tbl_Contacts ON (tbl_ContactCompanies.ContactCompanyID = tbl_Contacts.ContactCompanyID AND tbl_Contacts.IsDefaultContact = 1)
				INNER JOIN tbl_company_sites ON (tbl_company_sites.CompanySiteID=tbl_ContactCompanies.SystemSiteID) 
			where tbl_company_sites.CompanyID=@SystemSiteID AND tbl_ContactCompanies.IsDisabled=0  AND tbl_ContactCompanies.Iscustomer = 2 ;
